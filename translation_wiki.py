...
"""Method that performs the Wiki sql dataset loading and pre-processing. 
   Removes empty lines, apply preprocessing and tokenization"""
def create_wiki_dataset(file_train_desc, file_test_desc,
                         file_train_sql,file_test_sql):
    
    x_train = pd.read_csv(file_train_desc)
    y_train = pd.read_csv(file_train_sql)
    
    x_test = pd.read_csv(file_test_desc)
    y_test = pd.read_csv(file_test_sql)
    
    
    x_train = x_train.question.apply(str).apply(preprocess_sentence).values
    y_train = y_train.sql.apply(str).apply(preprocess_sql).values
    

    x_test = x_test.question.apply(str).apply(preprocess_sentence).values
    y_test = y_test.sql.apply(str).apply(preprocess_sql).values
...
def create_sketch_data(file_sketch_train, file_sketch_test, tokenizer):
    ...
    x_train = x_train.sql_sketch.apply(str).apply(preprocess_sentence).values
    x_test = x_test.sql_sketch.apply(str).apply(preprocess_sentence).values
...
# file paths
file_train_desc = 'WikiSql/sql_description_train_Wiki.csv'
file_test_desc = 'WikiSql/sql_description_test_Wiki.csv'
file_train_sql = 'WikiSql/sql_train_WIki.csv'
file_test_sql = 'WikiSql/sql_test_WIki.csv'
file_sketch_train = 'WikiSql/sql_sketch_train_Wiki.csv'
file_sketch_test = 'WikiSql/sql_sketch_test_Wiki.csv'

#tensors of data of normal data
input_tensor_train, input_tensor_test,\
    target_tensor_train, target_tensor_test, \
        inp_lang, targ_lang  = create_wiki_dataset(file_train_desc, file_test_desc,
                                                    file_train_sql,file_test_sql)
...
y_test = get_sql_test(file_test_sql)
